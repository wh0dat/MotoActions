...
public final class FlowableSubscribeOn<T> extends AbstractFlowableWithUpstream<T, T> {
    ...
    static final class SubscribeOnSubscriber<T> extends AtomicReference<Thread> implements Subscriber<T>, Subscription, Runnable {
        ...
        /* renamed from: s */
        final AtomicReference<Subscription> f326s = new AtomicReference<>();
        Publisher<T> source;
        final Worker worker;
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.setOnce(this.f326s, subscription)) {
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                Subscription subscription = (Subscription) this.f326s.get();
                if (subscription != null) {
                    requestUpstream(j, subscription);
                    return;
                }
                BackpressureHelper.add(this.requested, j);
                Subscription subscription2 = (Subscription) this.f326s.get();
        ...
        public void requestUpstream(final long j, final Subscription subscription) {
        ...
        public void cancel() {
            SubscriptionHelper.cancel(this.f326s);
            this.worker.dispose();
        }
    }

    public FlowableSubscribeOn(Publisher<T> publisher, Scheduler scheduler2, boolean z) {
        super(publisher);
