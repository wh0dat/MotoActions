...
public class FPSSlideAppSettingsUpdater extends SettingsUpdater {
    private static final String ENABLE_SOURCE = "actions_fps_slide_app_enable_source";

    private static class SingletonHolder {
        /* access modifiers changed from: private */
        public static final SettingsUpdater INSTANCE = new FPSSlideAppSettingsUpdater();
    ...
    public void toggleStatus(boolean z, boolean z2, String str) {
        ignoreDiscoveryStatus(FeatureKey.FPS_SLIDE_APP, z);
        setEnabledSource(z, str);
        FPSSlideHelper.setScrollOnApp(z);
        if (z) {
            DiscoveryManager.getInstance().cancelFDN(FeatureKey.FPS_SLIDE_APP);
        }
        ActionsSettingsProvider.hideCard(ContainerProviderItemFPSSlideApp.PRIORITY_KEY);
        ActionsSettingsProvider.notifyChange(ContainerProviderItemFPSSlideApp.TABLE_NAME);
    }

    public boolean getDefaultSettingsValue() {
        return FeatureKey.FPS_SLIDE_APP.getEnableDefaultState();
    }
}
